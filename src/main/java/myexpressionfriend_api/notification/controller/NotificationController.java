...
public class NotificationController {
    ...
    @Operation(summary = "SSE 알림 구독", description = "실시간 알림 수신을 위한 SSE 스트림을 구독합니다.")
    ...
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "전체 알림 읽음 처리", description = "로그인한 사용자의 모든 알림을 읽음으로 처리합니다.")
    ...
    @Operation(summary = "오래된 알림 삭제 (ADMIN)", description = "지정한 일수(daysAgo) 이전의 알림을 일괄 삭제합니다. 기본값 90일.")
    public ResponseEntity<ApiResponse<Integer>> cleanup(
        ...
        return ResponseEntity.ok(ApiResponse.success(deleted + "개의 알림이 삭제되었습니다.", deleted));
    }
}
