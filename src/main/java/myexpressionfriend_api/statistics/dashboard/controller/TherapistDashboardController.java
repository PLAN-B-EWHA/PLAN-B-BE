...
@Tag(name = "치료사 - 대시보드", description = "치료사가 담당 아동의 학습 요약, 진행도, 게임 기록을 조회하는 API")
public class TherapistDashboardController {
    ...
    @Operation(summary = "표정 학습 요약 조회", description = "담당 아동의 표정 인식 성공률, 유창성, 재시도 경향 등 치료사용 요약 통계를 조회합니다.")
    public ResponseEntity<ApiResponse<ExpressionSummaryDto>> getExpressionSummary(
        ...
        ExpressionSummaryDto summary = therapistDashboardService.getExpressionSummary(userId, childId);
        return ResponseEntity.ok(ApiResponse.success(summary));
    }

    @GetMapping("/children/{childId}/dialogue/summary")
    @Operation(summary = "대화 학습 요약 조회", description = "담당 아동의 대화 주제별 점수율, 라포 지수, 반복 패턴 등 치료사용 요약 통계를 조회합니다.")
    ...
    @Operation(summary = "대화 학습 진행도 조회", description = "담당 아동의 대화 학습 점수와 참여 추이를 조회합니다.")
    public ResponseEntity<ApiResponse<DialogueProgressDto>> getDialogueProgress(
            @PathVariable UUID childId,
            Authentication authentication) {
        UUID userId = SecurityContextUtil.getCurrentUserId(authentication);
        return ResponseEntity.ok(ApiResponse.success(
                therapistDashboardService.getDialogueProgress(userId, childId)));
    }

    @GetMapping("/children/{childId}/dialogue/history")
    @Operation(summary = "대화 게임 기록 조회", description = "담당 아동의 대화 게임 세션 기록을 최신순으로 조회합니다. theme 값으로 주제를 필터링할 수 있습니다.")
    ...
    @Operation(summary = "표정 게임 기록 조회", description = "담당 아동의 표정 게임 세션 기록을 최신순으로 조회합니다. emotion 값으로 감정 목표를 필터링할 수 있습니다.")
